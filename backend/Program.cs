...
// 2) Si no existe, probar wwwroot en las mismas ubicaciones
var currentDir = Directory.GetCurrentDirectory();
var exeDir = Path.GetDirectoryName(Environment.ProcessPath ?? string.Empty) ?? string.Empty;

var candidatePaths = new[]
{
    Path.Combine(currentDir, "wwwroot", "browser"),
    Path.Combine(exeDir, "wwwroot", "browser"),
...
var wwwRootPath = candidatePaths.FirstOrDefault(Directory.Exists);
var hasStaticFiles = !string.IsNullOrWhiteSpace(wwwRootPath) && Directory.Exists(wwwRootPath);
...
{
    Console.WriteLine("[STATIC] No se encontrÐ˜ la carpeta wwwroot; modo sÐ¡Ã–lo API");
}
...
app.MapPost("/api/auth/login", async (LoginRequest request, HttpContext context) =>
{
    ...
    await EnsureDefaultAdminAsync(connection);
    ...
    if (!await reader.ReadAsync())
    {
        ...
        return Results.Json(new { success = false, error = "Credenciales invÃ¡lidas" }, statusCode: StatusCodes.Status401Unauthorized);
    }
    ...
    if (!isValid)
    {
        await LogAccessAttemptAsync(connection, user.Id, user.Username, "login", ipAddress, userAgent, false, "ContraseÃ±a incorrecta");
        return Results.Json(new { success = false, error = "Credenciales invÃ¡lidas" }, statusCode: StatusCodes.Status401Unauthorized);
    }
...
app.MapPost("/api/nutrition/history", async (HistoryRequest request, HttpContext httpContext) =>
{
    try
    {
        Console.WriteLine("=== INICIO GUARDADO DE HISTORIA CLÍNICA ===");
        ...
        Console.WriteLine($"Paciente: {request.PersonalData?.Nombre}, Cédula: {request.PersonalData?.NumeroCedula}");

        if (!TryGetToken(httpContext, out var token) || token is null || !IsTokenValid(token, activeSessions, out var user))
        {
            Console.WriteLine("ERROR: Token inválido o no autorizado");
            return Results.Unauthorized();
        }

        Console.WriteLine($"Usuario autenticado: {user}");

        request = SanitizeHistoryRequest(request);

        await using var connection = new NpgsqlConnection(connectionString);
        await connection.OpenAsync();
        Console.WriteLine("Conexión a DB abierta");
        ...
        Console.WriteLine($"Paciente ID: {pacienteId}");
    ...
    {
        Console.WriteLine($"ERROR EN GUARDADO: {ex.GetType().Name}");
        ...
        return Results.Problem(
            detail: ex.Message,
            statusCode: 500,
            title: "Error al guardar la historia clínica"
        );
    }
});
...
app.MapGet("/api/nutrition/pacientes", async (HttpContext httpContext) =>
{
    ...
    // Para cada paciente, obtener sus historias clÃ­nicas
...
{
    // Endpoint temporal - eliminar despuÃ©s del uso
    await using var connection = new NpgsqlConnection(connectionString);
    await connection.OpenAsync();

    var deletedCounts = new Dictionary<string, int>();

    // Eliminar en orden correcto respetando claves forÃ¡neas
...
app.MapPost("/api/nutrition/ejecutar-sql", async (HttpContext httpContext) =>
{
    ...
    try
    {
        ...
        if (request == null)
        {
            return Results.BadRequest("No se recibiÃ³ el SQL");
        }

        var sqlCommand = request.RootElement.GetProperty("sql").GetString();
        if (string.IsNullOrWhiteSpace(sqlCommand))
        {
            return Results.BadRequest("SQL vacÃ­o");
        }
...
app.MapGet("/api/nutrition/pacientes/detallados", async (HttpContext httpContext) =>
{
    ...
    // Obtener todas las historias clÃ­nicas con datos del paciente
    await using var cmdHistorias = new NpgsqlCommand(
        @"SELECT 
    ...
    // Para cada historia clÃ­nica, obtener sus datos relacionados
    foreach (var historia in historiasConPaciente)
    {
        ...
        // Crear un reporte por cada historia clÃ­nica (paciente + fecha)
        reportesDetallados.Add(new
        {
            ...
            // Datos relacionados con esta historia especÃ­fica
...
// Obtener datos completos de un paciente especÃ­fico para ediciÃ³n
app.MapGet("/api/nutrition/pacientes/{id}", async (string id, HttpContext httpContext) =>
{
    ...
    {
        return Results.BadRequest(new { error = "ID de paciente invÃ¡lido" });
    }
...
app.MapDelete("/api/nutrition/pacientes/{id}", async (string id, HttpContext httpContext) =>
{
    if (!TryGetToken(httpContext, out var token) || token is null || !IsTokenValid(token, activeSessions, out var user))
    {
        return Results.Unauthorized();
    ...
    if (!Guid.TryParse(id, out var pacienteGuid))
    {
        return Results.BadRequest(new { error = "ID de paciente invalido" });
    }
    ...
    await using var checkCmd = new NpgsqlCommand(
        @"SELECT nombre FROM pacientes WHERE id = @id LIMIT 1",
        connection, tx);
    checkCmd.Parameters.AddWithValue("id", pacienteGuid);
    ...
    var nombre = reader.IsDBNull(0) ? null : reader.GetString(0);
    await reader.CloseAsync();

    await using var countHistoriasCmd = new NpgsqlCommand(
        @"SELECT COUNT(*) FROM historias_clinicas WHERE paciente_id = @id",
        connection, tx);
    countHistoriasCmd.Parameters.AddWithValue("id", pacienteGuid);
    var historiasCount = Convert.ToInt32(await countHistoriasCmd.ExecuteScalarAsync());

    await using var deleteCmd = new NpgsqlCommand(
        @"DELETE FROM pacientes WHERE id = @id",
        connection, tx);
    deleteCmd.Parameters.AddWithValue("id", pacienteGuid);

    var deleted = await deleteCmd.ExecuteNonQueryAsync();
    await tx.CommitAsync();

    Console.WriteLine($"[DB] Paciente eliminado: {pacienteGuid} ({nombre ?? "sin nombre"}), historias eliminadas: {historiasCount}");

    return Results.Ok(new
    {
        success = deleted > 0,
        message = "Paciente eliminado correctamente",
        historiasEliminadas = historiasCount
    });
});
...
app.MapGet("/api/nutrition/pacientes/buscar/cedula/{cedula}", async (string cedula, HttpContext httpContext) =>
{
    ...
    {
        return Results.BadRequest(new { error = "NÃºmero de cÃ©dula requerido" });
    }
...
// Buscar paciente por cÃ©dula con su Ãºltima historia clÃ­nica completa
app.MapGet("/api/nutrition/pacientes/buscar/cedula/{cedula}/ultima-historia", async (string cedula, HttpContext httpContext) =>
{
    ...
    // Buscar la Ãºltima historia clÃ­nica
    ...
    // Obtener datos de la Ãºltima historia
    object? antecedentes = null;
...
// Obtener datos completos de una historia clÃ­nica especÃ­fica para ediciÃ³n
app.MapGet("/api/nutrition/historias/{id}", async (string id, HttpContext httpContext) =>
{
    ...
    {
        return Results.BadRequest(new { error = "ID de historia invalido" });
    }

    await using var connection = new NpgsqlConnection(connectionString);
    await connection.OpenAsync();

    // Obtener datos bÃ¡sicos de la historia
    await using var cmdHistoria = new NpgsqlCommand(
    ...
    if (!await reader.ReadAsync())
    {
        return Results.NotFound(new { error = "Historia clÃ­nica no encontrada" });
    }
    ...
    // Obtener datos antropomÃ©tricos
    await using var cmdAntro = new NpgsqlCommand(
...
// Actualizar historia clÃ­nica completa
app.MapPut("/api/nutrition/historias/{id}", async (string id, HistoryRequest request, HttpContext httpContext) =>
{
    ...
    try
    {
        // Primero, obtener el paciente_id de la historia clÃ­nica
        Guid pacienteId;
        ...
        // Actualizar datos del paciente si estÃ¡n presentes
        ...
        // Actualizar historia clÃ­nica bÃ¡sica
        await using var cmdHistoria = new NpgsqlCommand(
        ...
        // Actualizar o insertar datos antropomÃ©tricos
        ...
        if (datoAntro != null)
        {
            ...
            {
                // Insertar nuevos datos antropomÃ©tricos
        ...
        // Actualizar o insertar hÃ¡bitos
        ...
        if (valoresBio != null)
        {
            ...
            {
                // Insertar nuevos valores bioquÃ­micos
        ...
        return Results.Ok(new { success = true, message = "Historia clÃ­nica actualizada correctamente" });
    }
    catch (Exception ex)
    {
        await transaction.RollbackAsync();
        Console.Error.WriteLine($"Error actualizando historia: {ex.Message}");
        return Results.Json(new { error = "Error al actualizar la historia clÃ­nica", details = ex.Message }, statusCode: 500);
    }
});

// Eliminar una historia clínica específica
app.MapDelete("/api/nutrition/historias/{id}", async (string id, HttpContext httpContext) =>
{
    if (!TryGetToken(httpContext, out var token) || token is null || !IsTokenValid(token, activeSessions, out var user))
    {
        return Results.Unauthorized();
    ...
    try
    {
        // Verificar que la historia existe y obtener información del paciente
        await using var checkCmd = new NpgsqlCommand(
            @"SELECT h.id, p.nombre, h.fecha_consulta 
              FROM historias_clinicas h
              INNER JOIN pacientes p ON h.paciente_id = p.id
              WHERE h.id = @id",
            connection, transaction);
        checkCmd.Parameters.AddWithValue("id", historiaGuid);
        ...
        var pacienteNombre = reader.IsDBNull(1) ? "Desconocido" : reader.GetString(1);
        var fechaConsulta = reader.IsDBNull(2) ? null : reader.GetDateTime(2).ToString("yyyy-MM-dd");
        await reader.CloseAsync();

        // Eliminar la historia clínica (las tablas relacionadas se eliminarán en cascada)
        await using var deleteCmd = new NpgsqlCommand(
            @"DELETE FROM historias_clinicas WHERE id = @id",
            connection, transaction);
        deleteCmd.Parameters.AddWithValue("id", historiaGuid);
        ...
        await transaction.CommitAsync();
        Console.WriteLine($"[DELETE] Historia clínica eliminada: {historiaGuid} - Paciente: {pacienteNombre}, Fecha: {fechaConsulta}");
    ...
    {
        await transaction.RollbackAsync();
        Console.Error.WriteLine($"[ERROR] Error eliminando historia clínica: {ex.Message}");
        return Results.Json(new { error = "Error al eliminar la historia clínica", details = ex.Message }, statusCode: 500);
    }
});

// ====== ENDPOINTS PLANES NUTRICIONALES ======

// GET - Obtener todos los planes de una historia clÃ­nica
...
// POST - Crear un nuevo plan nutricional con su alimentaciÃ³n semanal
app.MapPost("/api/nutrition/planes", async (HttpContext httpContext) =>
{
    ...
    if (request == null)
    {
        return Results.BadRequest("Datos invÃ¡lidos");
    }
    ...
    try
    {
        ...
        // Insertar alimentaciÃ³n semanal si existe
...
// GET - Obtener la alimentaciÃ³n semanal de un plan
...
// =============================================
// ENDPOINTS PARA PLANES DE ALIMENTACIÃ“N
// =============================================

// GET - Listar historias clÃ­nicas disponibles
...
// POST - Guardar plan de alimentaciÃ³n completo
app.MapPost("/api/planes", async (HttpContext httpContext) =>
{
    ...
    if (request == null)
    {
        return Results.BadRequest(new { error = "Datos invÃ¡lidos" });
    }
...
// PUT - Actualizar plan de alimentaciÃ³n existente
app.MapPut("/api/planes/{planId}", async (HttpContext httpContext, string planId) =>
{
    ...
    try
    {
        ...
        // Eliminar alimentaciÃ³n existente
...
// GET - Obtener plan por ID con alimentaciÃ³n
app.MapGet("/api/planes/{planId}", async (HttpContext httpContext, string planId) =>
{
    ...
    // Mapear dÃ­as numÃ©ricos a nombres
...
static string ResolveConnectionString(IConfiguration configuration)
{
    ...
    {
        var exeConnectionFile = Path.Combine(exeDir, "database", "connection.local");
        if (File.Exists(exeConnectionFile))
        {
            var text = File.ReadAllText(exeConnectionFile).Trim();
            if (!string.IsNullOrWhiteSpace(text))
            {
                Console.WriteLine($"[DB] Using connection from file: {exeConnectionFile}");
                return text;
            }
        }
    }
...
static async Task EnsureDefaultAdminAsync(NpgsqlConnection connection)
{
    const string username = "admin";
    const string email = "admin@nutriweb.com";
    ...
    if (existingId == Guid.Empty)
    {
        ...
        await using var insertCmd = new NpgsqlCommand(
            @"INSERT INTO usuarios (id, username, email, nombre, password_hash, rol, activo, fecha_creacion)
              VALUES (@id, @username, @email, @nombre, @hash, 'admin', true, NOW())",
            connection);
        ...
        Console.WriteLine("[AUTH] Usuario admin creado por defecto");
        return;
    }
    ...
    if (string.IsNullOrWhiteSpace(storedHash))
    {
        ...
        Console.WriteLine("[AUTH] Usuario admin actualizado con contrasena por defecto");
    }
}
...
static async Task<Guid> InsertHistoriaClinicaAsync(NpgsqlConnection connection, NpgsqlTransaction tx, Guid pacienteId, HistoryRequest request)
{
    ...
    var id = (Guid)(await cmd.ExecuteScalarAsync() ?? throw new InvalidOperationException("No se pudo insertar la historia clÃ­nica"));
    return id;
}
...
static async Task InsertFrecuenciaAsync(NpgsqlConnection connection, NpgsqlTransaction tx, Guid historiaId, IDictionary<string, string>? frequency)
{
    ...
    foreach (var entry in frequency)
    {
        ...
        var categoria = LegacyEncodingSanitizer.Sanitize(parts.Length > 1 ? parts[0] : "General") ?? string.Empty;
        var alimento = LegacyEncodingSanitizer.Sanitize(parts.Length > 1 ? parts[1] : parts[0]) ?? string.Empty;
        var frecuencia = LegacyEncodingSanitizer.Sanitize(entry.Value) ?? string.Empty;
...
static HistoryRequest SanitizeHistoryRequest(HistoryRequest request)
{
    string? Clean(string? value) => LegacyEncodingSanitizer.Sanitize(value);

    return new HistoryRequest
    {
        PersonalData = request.PersonalData is null ? null : new PersonalData
        {
            Nombre = Clean(request.PersonalData.Nombre),
            EdadCronologica = Clean(request.PersonalData.EdadCronologica),
            Sexo = Clean(request.PersonalData.Sexo),
            LugarResidencia = Clean(request.PersonalData.LugarResidencia),
            EstadoCivil = Clean(request.PersonalData.EstadoCivil),
            NumeroCedula = Clean(request.PersonalData.NumeroCedula),
            Telefono = Clean(request.PersonalData.Telefono),
            Ocupacion = Clean(request.PersonalData.Ocupacion),
            Email = Clean(request.PersonalData.Email),
            FechaConsulta = Clean(request.PersonalData.FechaConsulta),
        },
        MotivoConsulta = Clean(request.MotivoConsulta),
        Antecedentes = request.Antecedentes is null ? null : new AntecedentesData
        {
            APF = Clean(request.Antecedentes.APF),
            APP = Clean(request.Antecedentes.APP),
            APQ = Clean(request.Antecedentes.APQ),
            AGO = Clean(request.Antecedentes.AGO),
            Menarquia = Clean(request.Antecedentes.Menarquia),
            P = Clean(request.Antecedentes.P),
            G = Clean(request.Antecedentes.G),
            C = Clean(request.Antecedentes.C),
            A = Clean(request.Antecedentes.A),
            Alergias = Clean(request.Antecedentes.Alergias)
        },
        Diagnostico = Clean(request.Diagnostico),
        Habitos = request.Habitos is null ? null : new HabitosData
        {
            Fuma = Clean(request.Habitos.Fuma),
            Alcohol = Clean(request.Habitos.Alcohol),
            Cafe = Clean(request.Habitos.Cafe),
            Hidratacion = Clean(request.Habitos.Hidratacion),
            Gaseosas = Clean(request.Habitos.Gaseosas),
            ActividadFisica = Clean(request.Habitos.ActividadFisica),
            Te = Clean(request.Habitos.Te),
            Edulcorantes = Clean(request.Habitos.Edulcorantes),
            Alimentacion = Clean(request.Habitos.Alimentacion)
        },
        SignosVitales = request.SignosVitales is null ? null : new SignosData
        {
            PresionArterial = Clean(request.SignosVitales.PresionArterial),
            Temperatura = Clean(request.SignosVitales.Temperatura),
            FrecuenciaCardiaca = Clean(request.SignosVitales.FrecuenciaCardiaca),
            FrecuenciaRespiratoria = Clean(request.SignosVitales.FrecuenciaRespiratoria)
        },
        DatosAntropometricos = request.DatosAntropometricos is null ? null : new AntropometricosData
        {
            Edad = Clean(request.DatosAntropometricos.Edad),
            EdadMetabolica = Clean(request.DatosAntropometricos.EdadMetabolica),
            Sexo = Clean(request.DatosAntropometricos.Sexo),
            Peso = Clean(request.DatosAntropometricos.Peso),
            MasaMuscular = Clean(request.DatosAntropometricos.MasaMuscular),
            GrasaCorporalPorcentaje = Clean(request.DatosAntropometricos.GrasaCorporalPorcentaje),
            GrasaCorporal = Clean(request.DatosAntropometricos.GrasaCorporal),
            Talla = Clean(request.DatosAntropometricos.Talla),
            GrasaVisceralPorcentaje = Clean(request.DatosAntropometricos.GrasaVisceralPorcentaje),
            IMC = Clean(request.DatosAntropometricos.IMC),
            KcalBasales = Clean(request.DatosAntropometricos.KcalBasales),
            ActividadFisica = Clean(request.DatosAntropometricos.ActividadFisica),
            CircunferenciaCintura = Clean(request.DatosAntropometricos.CircunferenciaCintura),
            CircunferenciaCadera = Clean(request.DatosAntropometricos.CircunferenciaCadera),
            CircunferenciaBrazo = Clean(request.DatosAntropometricos.CircunferenciaBrazo),
            Pantorrilla = Clean(request.DatosAntropometricos.Pantorrilla),
            CircunferenciaMuslo = Clean(request.DatosAntropometricos.CircunferenciaMuslo),
            PesoAjustado = Clean(request.DatosAntropometricos.PesoAjustado),
            FactorActividadFisica = Clean(request.DatosAntropometricos.FactorActividadFisica),
            TiemposComida = Clean(request.DatosAntropometricos.TiemposComida)
        },
        ValoresBioquimicos = request.ValoresBioquimicos is null ? null : new BioquimicosData
        {
            Glicemia = Clean(request.ValoresBioquimicos.Glicemia),
            ColesterolTotal = Clean(request.ValoresBioquimicos.ColesterolTotal),
            Trigliceridos = Clean(request.ValoresBioquimicos.Trigliceridos),
            HDL = Clean(request.ValoresBioquimicos.HDL),
            LDL = Clean(request.ValoresBioquimicos.LDL),
            TGO = Clean(request.ValoresBioquimicos.TGO),
            TGP = Clean(request.ValoresBioquimicos.TGP),
            Urea = Clean(request.ValoresBioquimicos.Urea),
            Creatinina = Clean(request.ValoresBioquimicos.Creatinina)
        },
        Recordatorio24h = request.Recordatorio24h is null ? null : new Recordatorio24hData
        {
            Desayuno = Clean(request.Recordatorio24h.Desayuno),
            Snack1 = Clean(request.Recordatorio24h.Snack1),
            Almuerzo = Clean(request.Recordatorio24h.Almuerzo),
            Snack2 = Clean(request.Recordatorio24h.Snack2),
            Cena = Clean(request.Recordatorio24h.Cena),
            Extras = Clean(request.Recordatorio24h.Extras)
        },
        Frequency = request.Frequency?.Where(kvp => !string.IsNullOrWhiteSpace(kvp.Key))
            .ToDictionary(
                kvp => Clean(kvp.Key) ?? string.Empty,
                kvp => Clean(kvp.Value) ?? string.Empty),
        NotasExtras = Clean(request.NotasExtras)
    };
}
...
static object DbString(string? value)
{
    ...
    var sanitized = LegacyEncodingSanitizer.Sanitize(value);
    return string.IsNullOrEmpty(sanitized) ? DBNull.Value : sanitized;
}
...
static class LegacyEncodingSanitizer
{
    private static readonly Encoding Win1252Encoding = Encoding.GetEncoding(
        1252,
        new EncoderReplacementFallback("?"),
        new DecoderExceptionFallback());

    private static readonly Dictionary<char, string> SymbolReplacements = new()
    {
        ['\u2265'] = ">=",
        ['\u2264'] = "<=",
        ['\u2260'] = "!=",
    ...
    public static string? Sanitize(string? value)
    {
        ...
        {
            if (SymbolReplacements.TryGetValue(ch, out var replacement))
            {
        ...
        var encoded = Win1252Encoding.GetBytes(sb.ToString());
        return Win1252Encoding.GetString(encoded);
    }
}
