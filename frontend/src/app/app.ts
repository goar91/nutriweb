...
export class App implements OnInit, OnDestroy {
  ...
  protected readonly currentPacienteId = signal<string | null>(null);
  ...
  private buscarYAutocompletarPorCedula(cedula: string): void {
    this.currentPacienteId.set(null);

    this.nutritionService.buscarPacienteConUltimaHistoria(cedula).subscribe({
      next: (data: any) => {
        console.log('Paciente encontrado con cedula:', cedula, data);
        this.currentPacienteId.set(data.paciente?.id ?? null);
      ...
      error: (error) => {
        if (error.status !== 404) {
          console.error('Error buscando paciente por cedula:', error);
        }
        this.currentPacienteId.set(null);
      }
    });
  ...
  private loadDataForEdit(editContext: any): void {
    ...
    if (editContext?.type === 'historia' && editContext.data) {
      ...
      this.currentPacienteId.set(editContext.data.paciente_id ?? null);
  ...
  private loadHistoriaData(data: any): void {
    ...
    this.currentPacienteId.set(data?.paciente_id ?? null);
  ...
  private loadPacienteData(data: any): void {
    this.currentPacienteId.set(data?.id ?? this.editingId());
    this.nutriForm.patchValue({
      personalData: {
        nombre: data.nombre || '',
  ...
  protected deleteHistoriaClinica(): void {
    const historiaId = this.editingId();
    if (!historiaId || this.isSubmitting() || this.editType() !== 'historia') {
      return;
    }
    ...
    const confirmed = confirm(
      ...
      'El paciente NO será eliminado y permanecerá en el sistema.\n\n' +
      'Se borrarán:\n' +
    ...
    this.nutritionService.deleteHistoriaClinica(historiaId).subscribe({
      next: (response: any) => {
        ...
        this.resultMessage.set(
          ...
          `Paciente: ${response.paciente || 'N/A'}\n` +
      ...
      error: (error) => {
        ...
        this.resultMessage.set(
          'No se pudo eliminar la historia clínica. Error: ' + (error.error?.error || error.message)
        );
        this.isSubmitting.set(false);
      }
    });
  ...
  protected deletePaciente(): void {
    const pacienteId = this.currentPacienteId();
    if (!pacienteId || this.isSubmitting()) {
      return;
    }

    const confirmed = confirm(
      '⚠️ ELIMINAR PACIENTE COMPLETO ⚠️\n\n' +
      'Esta acción eliminará al paciente Y TODAS sus historias clínicas.\n' +
      'Esta acción NO se puede deshacer.\n\n' +
      '¿Estás seguro?'
    );
    if (!confirmed) {
    ...
    this.nutritionService.deletePaciente(pacienteId).subscribe({
      next: (response: any) => {
        const historiasCount = Number(response?.historiasEliminadas ?? 0);
        const suffix = historiasCount === 1 ? 'historia clínica' : 'historias clínicas';
        const message =
          historiasCount > 0
            ? `Paciente eliminado junto con ${historiasCount} ${suffix}.`
      ...
      error: (error) => {
        ...
        this.resultMessage.set(
          'No se pudo eliminar el paciente. Error: ' + (error.error?.error || error.message)
        );
        this.isSubmitting.set(false);
      }
    });
  ...
  protected getFrequency(category: string, food: string): string {
    return this.frequencySelections[this.frequencyKey(category, food)] ?? this.supportedFrequencies[0];
  }
